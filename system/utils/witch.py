...
class Witch:
    ...
    def brew_poison(self, model, trainloader, last_poison_delta=None):
        ...
        poisonloader = DataLoader(subset, batch_size=8, shuffle=False)
    ...
    def brew_camou(self, model,trainloader, last_camou_delta=None):
        ...
        if len(self.camou_index) > 0:
            ...
            for trial in range(self.args.camouflage_restarts):
                ...
                camouloader=DataLoader(subset, batch_size=8, shuffle=False, drop_last=True)
                ...
                for iter in range(self.args.camouflage_attackiter):
                    ...
                    with torch.no_grad():
                        ...
                        camou_delta.data = torch.max(
                            torch.min(camou_delta, self.args.camouflage_eps / self.std_tensor / 255),
                            -self.args.camouflage_eps / self.std_tensor / 255)
                        camou_delta.data = torch.max(
                            torch.min(camou_delta, (1 - self.mean_tensor) / self.std_tensor - camou_bounds),
                            -self.mean_tensor / self.std_tensor - camou_bounds)
