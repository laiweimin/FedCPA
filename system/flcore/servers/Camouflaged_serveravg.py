        ...
        if self.unlearning == "NoUnlearning":
            ...
            if self.unlearning == "FedRecovery":
                ...
                if self.unlearning_type == "UnlearningData":
                    ...
                    # for i in range(self.global_rounds+1):
                    #     p_i = squared_norms[:i+1] / torch.sum(squared_norms[:i+1])
                    #     if i ==0:
                    #         Recovery_parameters = self.global_flattened_parameters_logs[i+1] - gradient_residuals[0]
                    #     else:
                    #         Recovery_parameters=self.global_flattened_parameters_logs[i+1] - torch.sum(torch.stack(gradient_residuals[:i+1]) * p_i[:, None], dim=0)
                    #     param_index = 0
                    ...
                    #
                    #     Recovery_parameters = Recovery_parameters * 4 / 5 + torch.sum(flattened_parameters,
                    #                                                                   dim=0) / 2/ 5
                    #
                    ...
                    Recovery_parameters=self.global_flattened_parameters_logs[-1].cuda() - torch.sum(torch.stack(gradient_residuals).cuda() * p_i[:, None], dim=0)

                    Recovery_parameters = Recovery_parameters + noise

                    self.load_flattened_parameters(Recovery_parameters, self.global_model)
                    # param_index = 0
                    # for server_param in self.global_model.parameters():
                    #     param_size = torch.numel(server_param)
                    #     new_param_vector = Recovery_parameters[param_index:param_index + param_size]
                    ...
                    Recovery_parameters = self.global_flattened_parameters_logs[-1].cuda() - torch.sum(
                        torch.stack(gradient_residuals).cuda() * p_i[:, None], dim=0)
            ...
            if self.unlearning == "FedEraser":
                ...
                for i in range(1, self.global_rounds + 1):
                    ...
                    self.load_flattened_parameters(unlearn_global_models[-1].cuda(), self.global_model)
                    # param_index = 0
                    # for server_param in self.global_model.parameters():
                    #     param_size = torch.numel(server_param)
                    ...
                    if self.args.defense == 'TrimmedMean':
                        print("TrimmedMean")
                        newClientM = defences.trimmed_mean(flattened_parameters, 0.2).cpu()
                    elif self.args.defense == 'Krum':
                        print("Krum")
                        newClientM = defences.krum(flattened_parameters, len(flattened_parameters), 2).cpu()
                    elif self.args.defense == 'Bulyan':
                        print("Bulyan")
                        newClientM = defences.bulyan(flattened_parameters, len(flattened_parameters), 2).cpu()
                    elif self.args.defense == 'Median':
                        print("Median")
                        newClientM = defences.median(flattened_parameters).cpu()
                    else:
                        newClientM = torch.mean(flattened_parameters, dim=0).cpu()
                    ...
                    if self.unlearning_type == "UnlearningData":
                        self.load_flattened_parameters(return_model_state.cuda(), self.global_model)
                        # param_index = 0
                        # for server_param in self.global_model.parameters():
                        #     param_size = torch.numel(server_param)
                        ...
                        for model in uploaded_models:
                            ...
                            flattened_parameters.append(model_parameters_flattened)
                        flattened_parameters = torch.stack(flattened_parameters)
                        return_model_state = return_model_state * 4 / 5 + torch.mean(flattened_parameters, dim=0).cpu() / 5
            ...
            if self.unlearning == "FedRecover":
                ...
                for i in range(self.global_rounds+1):
                    s_t = time.time()
                    self.load_flattened_parameters(recover_golbal_model_logs[-1].cuda(), self.global_model)
                    # param_index = 0
                    # for server_param in self.global_model.parameters():
                    #     param_size = torch.numel(server_param)
                    ...
                    if i <= 3 or i % 5 == 0 or i >= self.global_rounds-2:
                        ...
                        exact_clients_model = []
                        recover_golbal_model_logs[-1] = recover_golbal_model_logs[-1].cuda()
                        for client in self.selected_clients:
                            client.train(i)
                        ...
                        recover_golbal_model_logs[-1] = recover_golbal_model_logs[-1].cpu()
                        recover_golbal_model_logs.append(mean_estimated_update.cpu())
                        exact_global_model_delta_logs.append(recover_golbal_model_logs[-1] - recover_golbal_model_logs[-2])
                    else:
                        print('---------------------')
                        Delta_W = torch.stack(exact_global_model_delta_logs[-2:]).cuda()
                        estimated_update=[]
                        for c in self.clients:
                            if self.unlearning_type=="UnlearningData" and c.id in self.camouflage_clients:
                                ...
                                model_parameters_flattened = torch.cat(model_parameters)
                                estimated_client_update = recover_golbal_model_logs[-1].cuda() - model_parameters_flattened
                                print(f"Epoch{i}, CamouClient {c.id} is retrined")
                                estimated_update.append(estimated_client_update.cpu())
                            if self.unlearning_type=="UnlearningClient" and c.id == self.camouflage_clients[1]:
                                ...
                                v=recover_golbal_model_logs[-1].cuda() - self.global_flattened_parameters_logs[i].cuda()
                                ...
                                original_client_update = original_client_update.cuda()
