...
class DNN_CreditScore(nn.Module):
    def __init__(self, input_dim=29, mid_dim=100, num_classes=10):
        super(DNN_CreditScore, self).__init__()
...
class Conv1DNet(nn.Module):
    """
    Input  : (batch, 600)              –– 600 binary product indicators
    Intern : (batch, C, L)             –– treat the 600‑vector as a 1‑D signal
    ...
    def __init__(self, num_classes: int = 100):
        ...
        # in_channels = 1  (we have one "signal"), out_channels = 64 filters
        self.conv1 = nn.Conv1d(1, 64, kernel_size=7, padding=3)     # L stays 600
        self.bn1   = nn.BatchNorm1d(64)

        # More expressive layer; halve length with stride‑2 pooling
        self.conv2 = nn.Conv1d(64, 128, kernel_size=5, padding=2)   # L still 600
        self.bn2   = nn.BatchNorm1d(128)
        self.pool  = nn.MaxPool1d(kernel_size=2)                    # L → 300

        # ── classifier head ───────────────────────────────────────────────────
        # Global Average Pooling → fixed 128‑D vector, independent of length
        self.gap   = nn.AdaptiveAvgPool1d(1)                        # (N,128,1)
        self.fc    = nn.Linear(128, num_classes)

    def forward(self, x):
        """
        x expected as (batch, 600)  – the same tensor you fed the DNN.
        """
        x = x.unsqueeze(1)                  # (N, 1, 600)

        x = F.relu(self.bn1(self.conv1(x))) # (N, 64, 600)
        x = self.pool(F.relu(self.bn2(self.conv2(x))))   # (N,128,300)
