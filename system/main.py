...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "mlr": # convex
            ...
            elif "Camelyon17" in args.dataset:
                args.model = FedAvgCNN(in_features=3, num_classes=args.num_classes, dim=10816).to(args.device)
            elif "COVIDx-0.1" in args.dataset:
                args.model = FedAvgCNN(in_features=3, num_classes=args.num_classes, dim=10816).to(args.device)
            else:
                args.model = FedAvgCNN(in_features=3, num_classes=args.num_classes, dim=1600).to(args.device)
            ...
            elif "Purchase" in args.dataset:
                args.model = DNN(600, 256, num_classes=args.num_classes).to(args.device)
            elif "CreditScore" in args.dataset:
                # args.model = DNN(29, 16, num_classes=args.num_classes).to(args.device)
                # args.model = DNN(46, 32, num_classes=args.num_classes).to(args.device)
                # args.model = DNN_CreditScore(29, num_classes=args.num_classes).to(args.device)
                args.model = DNN_CreditScore(46, num_classes=args.num_classes).to(args.device)
            elif "Camelyon17" in args.dataset:
                args.model = DNN(3*64*64, 100, num_classes=args.num_classes).to(args.device)
            elif "COVIDx-0.1" in args.dataset:
                args.model = DNN(3*64*64, 100, num_classes=args.num_classes).to(args.device)
            else:
                args.model = DNN(60, 20, num_classes=args.num_classes).to(args.device)

        elif model_str == "conv1d":
            if "Purchase" in args.dataset:
                args.model = Conv1DNet(num_classes=args.num_classes).to(args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-camoutype', "--camouflage_type", type=str, default="WithinClient", choices=["WithinClient", "CrossClient"])
    parser.add_argument('-cratio', "--camouflage_ratio", type=float, default=0.02)
    ...
    parser.add_argument('-cimages', "--camouflage_images_count", type=int, default=5)
    ...
    # UnlearningTypes
    parser.add_argument('-unl', "--unlearning", type=str, default="NoUnlearning",
                        choices=["NoUnlearning",  "Retrain", "FedEraser", "FedRecover", "FedRecovery", "FedForgotten", "AFUL"])
    parser.add_argument('-unlt', "--unlearning_type", type=str, default="UnlearningData",
                        choices=["UnlearningData", "UnlearningClient"])

    # DefenseTypes
    parser.add_argument('-def', "--defense", type=str, default="NoDefense",
                        choices=["NoDefense", "Krum", "TrimmedMean", "Bulyan", "LIE", "Median", "Fang", "Agr_merge", "Agr_xai", "FedREDefense", "ArgAmplifier"])
