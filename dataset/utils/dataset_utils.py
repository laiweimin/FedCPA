...
def save_file(config_path, train_path, test_path, train_data, test_data, num_clients, 
    ...
    for idx, train_dict in enumerate(train_data):
        if idx==0:
            trainset_x = train_dict['x']
            trainset_y = train_dict['y']
        else:
            trainset_x = np.vstack((trainset_x, train_dict['x']))
            trainset_y = np.hstack((trainset_y, train_dict['y']))
        with open(train_path + str(idx) + '.npz', 'wb') as f:
            np.savez_compressed(f, data=train_dict)
    with open(train_path + 'all.npz', 'wb') as f:
        trainset_all={'x':trainset_x, 'y':trainset_y}
        np.savez_compressed(f, data=trainset_all)

    for idx, test_dict in enumerate(test_data):
        if idx==0:
            testset_x = test_dict['x']
            testset_y = test_dict['y']
        else:
            testset_x = np.vstack((testset_x, test_dict['x']))
            testset_y = np.hstack((testset_y, test_dict['y']))
        with open(test_path + str(idx) + '.npz', 'wb') as f:
            np.savez_compressed(f, data=test_dict)
    with open(test_path + 'all.npz', 'wb') as f:
        testset_all={'x':testset_x, 'y':testset_y}
        np.savez_compressed(f, data=testset_all)
